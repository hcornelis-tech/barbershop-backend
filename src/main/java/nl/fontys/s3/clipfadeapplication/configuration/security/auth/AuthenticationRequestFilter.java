...
import nl.fontys.s3.clipfadeapplication.configuration.security.token.exception.InvalidAccessTokenException;
...
public class AuthenticationRequestFilter extends OncePerRequestFilter {
    ...
    private void sendAuthenticationError(HttpServletResponse response) throws IOException {
        logger.debug("Sending authentication error response");
        response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
        response.flushBuffer();
    }

    private void setupSpringSecurityContext(AccessToken accessToken) {
        UserDetails userDetails = new User(accessToken.getUserEmail(), "",
